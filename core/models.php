...
function insertCompany($pdo, $company_name, $country, $founded_year) {
    $sql = "INSERT INTO CarCompany (company_name, country, founded_year) VALUES (?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute(array($company_name, $country, $founded_year));
    if ($executeQuery) {
        return true;
    }
    return false;
...
function updateCompany($pdo, $company_id, $company_name, $country, $founded_year) {
    $sql = "UPDATE CarCompany SET company_name = ?, country = ?, founded_year = ? WHERE company_id = ?";
    $stmt = $pdo->prepare($sql);
